...
import com.example.lesiogotuje.ui.calculator.dilute.fluid.*;
...
public class Dilute {
    ...
    Dilute(final Action action, final double alcoholConcentration, final double solutionConcentration, final double value) {
        ...
        switch (action.getName()) {
            ...
            case "Ile alkoholu do wody": {
                first = new First(Type.WATER, Unit.MILILITR, value, this.alcoholConcentration);
                second = new Second(Type.ALCOHOL, Unit.MILILITR, value, this.waterConcetration);
                third = new Third(Type.SOLUTION, Unit.MILILITR, value, this.solutionConcentration);
                break;
            }
            case "Ile wody i alkoholu do roztworu": {
                first = new First(Type.SOLUTION, Unit.MILILITR, value, this.alcoholConcentration);
                second = new Second(Type.WATER, Unit.MILILITR, value, this.waterConcetration);
                third = new Third(Type.ALCOHOL, Unit.MILILITR, value, this.solutionConcentration);
                break;
            }
        }
        this.action = action;
    ...
    public void calculate() {
        ...
        if (action.getName().equals(Action.HOW_MUCH_WATER.getName())) {
            ...
            second.setValue(first.getValue() - first.getValue() / concentrationRatio);
            third.setValue(first.getValue() - second.getValue());
        }
    }
